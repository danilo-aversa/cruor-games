...
export async function openDarkPlaces(page) {
  await page.goto("/?cruorTest=dark-places&section=crucible&tool=darken&view=composer");
  await page.evaluate(() => {
    try {
  ...
  const crucibleButton = page.getByRole("button", { name: /crucible/i }).first();
...
export async function generateDarkPlace(page) {
  ...
  await expectVisible(page.getByTestId("dark-places-room-node").first(), { timeout: 20_000 });
}

export async function openRoomsMode(page) {
  const roomsButton = page.getByTestId("dark-places-toolbar-rooms-action").first();
...
export async function enterRoomsModeFromMapNode(page) {
  ...
  await expectVisible(page.getByTestId("dark-places-toolbar-rooms"), { timeout: 10_000 });
  await expectVisible(page.getByTestId("dark-places-room-inspector"));
  await expect(firstRoom).toHaveAttribute("aria-pressed", "true", { timeout: 10_000 });
  return firstRoom;
}
...
export async function addNextMissingRoomComponent(page, options = {}) {
  ...
  const hasAddButton = await isVisible(firstAddButton, options.addTimeout ?? 2_000);

  if (!hasAddButton) {
    await page.getByRole("button", { name: /close component navigator/i }).click();
...
export async function fillSelectedRoomUntilReadyOrBlocked(page, maxIterations = 3) {
  const added = [];
...
export async function openExportMode(page) {
  const exportButton = page.getByTestId("dark-places-toolbar-export-action").first();
