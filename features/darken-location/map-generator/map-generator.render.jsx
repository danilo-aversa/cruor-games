...
import { getCircleRaccordoCellsFromAnchor } from "./map-generator.circle-raccordo.js";
import {
...
.map-style-cruor .external-hatching-underlay .halo-buffer{stroke:var(--cruor-map-hatch-underlay-stroke,#050506)}
...
.map-style-cruor .door-symbols .door-panel,.map-style-cruor .corridor-junctions .junction-door-panel{fill:#21191d;stroke:var(--cruor-map-wall-main-stroke,#806b72)}
...
.map-style-ink .external-hatching-underlay .halo-buffer{stroke:var(--cruor-map-hatch-underlay-stroke,#dccaa6)}
...
.map-style-ink .door-symbols .door-panel,.map-style-ink .corridor-junctions .junction-door-panel{fill:#efe4ca;stroke:var(--cruor-map-wall-main-stroke,#1d1915)}
...
.map-style-cartographic .external-hatching-underlay .halo-buffer{stroke:var(--cruor-map-hatch-underlay-stroke,#f4efe4)}
...
.map-style-cartographic .door-symbols .door-panel,.map-style-cartographic .corridor-junctions .junction-door-panel{fill:#fff9eb;stroke:var(--cruor-map-wall-main-stroke,#1d1915)}
...
.map-style-blood .external-hatching-underlay .halo-buffer{stroke:var(--cruor-map-hatch-underlay-stroke,#d8c2ad)}
...
.map-style-blood .door-symbols .door-panel,.map-style-blood .corridor-junctions .junction-door-panel{fill:#e8d2be;stroke:var(--cruor-map-wall-main-stroke,#33030c)}
...
.map-style-bone .external-hatching-underlay .halo-buffer{stroke:var(--cruor-map-hatch-underlay-stroke,#e8dfcf)}
...
.map-style-bone .door-symbols .door-panel,.map-style-bone .corridor-junctions .junction-door-panel{fill:#f7efd8;stroke:var(--cruor-map-wall-main-stroke,#493523)}
...
.map-style-print .external-hatching-underlay .halo-buffer{stroke:var(--cruor-map-hatch-underlay-stroke,#fff)}
...
.map-style-print .door-symbols .door-panel,.map-style-print .corridor-junctions .junction-door-panel{fill:#fff;stroke:var(--cruor-map-wall-main-stroke,#000)}
...
export function getCorridorTypeClassName(corridor = {}, suffix = "") {
  const type = normalizeCorridorType(
    corridor?.corridorRenderProfile?.type || corridor?.corridorType,
    "normal",
  );
  const baseClass = corridor?.corridorRenderProfile?.renderClassName || `corridor-type-${type}`;
  return suffix ? `${baseClass}__${suffix}` : baseClass;
}

function getCorridorType(corridor = {}) {
  return normalizeCorridorType(
    corridor?.corridorRenderProfile?.type || corridor?.corridorType,
    "normal",
  );
}

function getCorridorCellCenter(cell, gridSize) {
  return {
    x: (cell.x + 0.5) * gridSize,
    y: (cell.y + 0.5) * gridSize,
  };
...
function getCorridorCellNormal(cells, index) {
  const direction = getCorridorAccentDirection(cells, index);
  return { x: -direction.y, y: direction.x };
}

function shouldRenderCorridorTypeAccent(corridor = {}) {
  const type = getCorridorType(corridor);
  return type !== "normal" && !corridor?.isRoomLink && !isOrganicCorridor(corridor);
}

function renderCollapsedCorridorRubble(corridor, generatedMap, cells, gridSize) {
  const seed = generatedMap?.config?.seed || "cruor-map";
  return cells
    .map((cell, index) => {
      const roll = hashStringToSeed(seed, corridor.id, cell.x, cell.y, "collapsed-rubble") % 100;
      if (roll >= 42) return null;
      const cx = (cell.x + 0.33 + (roll % 23) / 72) * gridSize;
      const cy = (cell.y + 0.33 + ((roll * 7) % 23) / 72) * gridSize;
      const size = gridSize * (0.08 + (roll % 5) * 0.012);
      return (
        <path
          key={`corridor-collapsed-rubble-${corridor.id}-${cell.x}-${cell.y}-${index}`}
          className={getCorridorTypeClassName(corridor, "rubble")}
          d={`M${cx} ${cy - size}l${size} ${size}l${-size * 0.65} ${size * 0.9}l${-size * 0.9} ${-size * 0.55}Z`}
        />
      );
    })
    .filter(Boolean);
...
function renderGalleryCorridorOrnaments(corridor, cells, gridSize) {
  return cells
    .map((cell, index) => {
      if (index <= 0 || index >= cells.length - 1 || index % 2 !== 0) return null;
      const center = getCorridorCellCenter(cell, gridSize);
      const normal = getCorridorCellNormal(cells, index);
      const half = gridSize * 0.28;
      return (
        <line
          key={`corridor-gallery-ornament-${corridor.id}-${cell.x}-${cell.y}-${index}`}
          className={getCorridorTypeClassName(corridor, "ornament")}
          x1={center.x - normal.x * half}
          y1={center.y - normal.y * half}
          x2={center.x + normal.x * half}
          y2={center.y + normal.y * half}
        />
      );
    })
    .filter(Boolean);
...
function getDoorBoundaryCenterPoint(door) {
  if (!door) return null;
  const x1 = Number(door.x1);
  const y1 = Number(door.y1);
  const x2 = Number(door.x2);
  const y2 = Number(door.y2);
  if (!Number.isFinite(x1) || !Number.isFinite(y1) || !Number.isFinite(x2) || !Number.isFinite(y2)) return null;
  return { x: (x1 + x2) / 2, y: (y1 + y2) / 2 };
}

function getCorridorEndpointDoorCenters(corridor, generatedMap) {
  const doors = (generatedMap?.dungeonMask?.doorSegments || [])
    .filter((door) => door?.corridorId === corridor?.id)
    .map((door) => ({ door, point: getDoorBoundaryCenterPoint(door) }))
    .filter((entry) => entry.point);
  const byEndpoint = new Map(doors.map((entry) => [entry.door.endpoint || "", entry.point]));
  return {
    from: byEndpoint.get("from") || doors[0]?.point || null,
    to: byEndpoint.get("to") || doors.find((entry) => entry.point !== byEndpoint.get("from"))?.point || doors[1]?.point || null,
  };
}

function maybeAddEndpointPoint(points, point, atStart) {
  if (!point || !Number.isFinite(point.x) || !Number.isFinite(point.y)) return points;
  const compare = atStart ? points[0] : points[points.length - 1];
  if (!compare || pointDistance(point, compare) <= 0.01) return points;
  return atStart ? [point, ...points] : [...points, point];
}

function getNarrowCorridorCenterlinePoints(corridor, generatedMap, cells, gridSize) {
  const rawPoints = (Array.isArray(cells) ? cells : [])
    .map((cell) => getCorridorCellCenter(cell, gridSize))
    .filter((point) => Number.isFinite(point.x) && Number.isFinite(point.y));
  let points = rawPoints.filter((point, index) => index === 0 || pointDistance(point, rawPoints[index - 1]) > 0.01);
  if (points.length < 2) return points;

  const doorCenters = getCorridorEndpointDoorCenters(corridor, generatedMap);
  [doorCenters.from, doorCenters.to].filter(Boolean).forEach((doorPoint) => {
    const first = points[0];
    const last = points[points.length - 1];
    const attachAtStart = pointDistance(doorPoint, first) <= pointDistance(doorPoint, last);
    points = maybeAddEndpointPoint(points, doorPoint, attachAtStart);
  });
  return points;
}
...
function offsetPolylinePoints(points, offset) {
  ...
  return points.map((point, index) => {
    ...
    if (!previous && next) {
      const normal = getSegmentNormal(point, next);
      return { x: point.x + normal.x * offset, y: point.y + normal.y * offset };
    }
    if (previous && !next) {
      const normal = getSegmentNormal(previous, point);
      return { x: point.x + normal.x * offset, y: point.y + normal.y * offset };
    }
    const previousNormal = getSegmentNormal(previous, point);
    const nextNormal = getSegmentNormal(point, next);
    const bisector = normalizeVector({
      x: previousNormal.x + nextNormal.x,
      y: previousNormal.y + nextNormal.y,
    });
    const denominator = Math.max(0.38, Math.abs(bisector.x * previousNormal.x + bisector.y * previousNormal.y));
    const miter = offset / denominator;
    return { x: point.x + bisector.x * miter, y: point.y + bisector.y * miter };
  });
}

function buildNarrowCorridorFloorPath(corridor, generatedMap, cells, gridSize) {
  const centerlinePoints = getNarrowCorridorCenterlinePoints(corridor, generatedMap, cells, gridSize);
  if (centerlinePoints.length < 2) return "";
  const offset = gridSize * 0.25;
  const left = offsetPolylinePoints(centerlinePoints, offset);
  const right = offsetPolylinePoints(centerlinePoints, -offset).reverse();
  const polygon = [...left, ...right].filter((point) => Number.isFinite(point?.x) && Number.isFinite(point?.y));
  if (polygon.length < 3) return "";
  return `${buildPolylinePath(polygon)}Z`;
}

function pointsToSegments(points = []) {
  const safePoints = points.filter((point) => Number.isFinite(point?.x) && Number.isFinite(point?.y));
  return safePoints.slice(1).map((point, index) => {
    const previous = safePoints[index];
    return { x1: previous.x, y1: previous.y, x2: point.x, y2: point.y };
  });
}

function buildRoughPointPath(points, config, seedPrefix, variant = "main") {
  return pointsToSegments(points)
    .map((segment, index) => createRoughWallPath(segment, config, `${seedPrefix}-${index}`, variant))
    .filter(Boolean)
    .join(" ");
}

function buildNarrowCorridorRailPaths(corridor, generatedMap, cells, gridSize) {
  const centerlinePoints = getNarrowCorridorCenterlinePoints(corridor, generatedMap, cells, gridSize);
  const offset = gridSize * 0.25;
  return [
    offsetPolylinePoints(centerlinePoints, offset),
    offsetPolylinePoints(centerlinePoints, -offset),
  ];
}

function getCorridorTypeBoundarySegments(corridor, generatedMap, gridSize) {
  const floorCells = getVisibleCorridorFloorCells(corridor, generatedMap);
  return computeBoundarySegments(floorCells, gridSize);
}

function getCorridorTypeWallReplacementSegments(generatedMap) {
  const config = generatedMap?.config || DEFAULT_CONFIG;
  const gridSize = config.gridSize || DEFAULT_CONFIG.gridSize;
  return (Array.isArray(generatedMap?.corridors) ? generatedMap.corridors : []).flatMap((corridor) => {
    if (!shouldRenderCorridorTypeAccent(corridor)) return [];
    const type = getCorridorType(corridor);
    if (type !== "narrow" && type !== "secret") return [];
    const floorCells = getVisibleCorridorFloorCells(corridor, generatedMap);
    if (floorCells.length === 0) return [];
    return getCorridorTypeBoundarySegments(corridor, generatedMap, gridSize);
  });
}

function removeCorridorTypeReplacementWalls(walls, generatedMap) {
  const replacements = getCorridorTypeWallReplacementSegments(generatedMap);
  if (replacements.length === 0) return walls;
  return walls.flatMap((wall) => {
    let parts = [wall];
    replacements.forEach((replacement) => {
      parts = parts.flatMap((part) => splitWallSegmentByGap(part, replacement));
    });
    return parts;
  });
}

function buildRoughSegmentPaths(segments, config, seedPrefix, variant = "main") {
  return segments
    .map((segment, index) =>
      createRoughWallPath(segment, config, `${seedPrefix}-${index}`, variant),
    )
    .filter(Boolean)
    .join(" ");
}

export function renderCorridorTypeWallAccents(generatedMap) {
  const corridors = Array.isArray(generatedMap?.corridors) ? generatedMap.corridors : [];
  const config = generatedMap?.config || DEFAULT_CONFIG;
  const gridSize = config.gridSize || DEFAULT_CONFIG.gridSize;
  const rendered = corridors
    .map((corridor) => {
      if (!shouldRenderCorridorTypeAccent(corridor)) return null;
      const type = getCorridorType(corridor);
      if (type !== "narrow" && type !== "secret") return null;
      const floorCells = getVisibleCorridorFloorCells(corridor, generatedMap);
      const cells = getVisibleCorridorTopologyCells(corridor, generatedMap);
      if (floorCells.length === 0 || cells.length === 0) return null;
      const wallReplacementSegments = getCorridorTypeBoundarySegments(corridor, generatedMap, gridSize);
      if (wallReplacementSegments.length === 0) return null;

      if (type === "narrow") {
        const railPointPaths = buildNarrowCorridorRailPaths(corridor, generatedMap, cells, gridSize);
        const railMainPaths = railPointPaths.map((points, index) =>
          buildRoughPointPath(points, config, `corridor-type-narrow-${corridor.id}-rail-${index}`, "main"),
        ).filter(Boolean);
        const railSketchPaths = railPointPaths.map((points, index) =>
          buildRoughPointPath(points, config, `corridor-type-narrow-sketch-${corridor.id}-rail-${index}`, "sketch"),
        ).filter(Boolean);
        if (railMainPaths.length < 2) return null;
        return (
          <g
            key={`corridor-type-wall-${corridor.id}`}
            className={`corridor-type-wall-accent ${getCorridorTypeClassName(corridor)}`}
            data-corridor-id={corridor.id}
            data-corridor-type={type}
            data-corridor-visual-width="half-cell"
          >
            <g className="wall-main corridor-type-narrow__wall-main">
              {railMainPaths.map((path, index) => (
                <path key={`narrow-rail-main-${corridor.id}-${index}`} d={path} />
              ))}
            </g>
            <g className="wall-sketch corridor-type-narrow__wall-sketch">
              {railSketchPaths.map((path, index) => (
                <path key={`narrow-rail-sketch-${corridor.id}-${index}`} d={path} />
              ))}
            </g>
          </g>
        );
      ...
      const secretMainPath = buildRoughSegmentPaths(
        wallReplacementSegments,
        config,
        `corridor-type-secret-${corridor.id}`,
        "main",
      );
      const secretSketchPath = buildRoughSegmentPaths(
        wallReplacementSegments,
        config,
        `corridor-type-secret-sketch-${corridor.id}`,
        "sketch",
      );
      if (!secretMainPath) return null;
      return (
        <g
          key={`corridor-type-wall-${corridor.id}`}
          className={`corridor-type-wall-accent ${getCorridorTypeClassName(corridor)}`}
          data-corridor-id={corridor.id}
          data-corridor-type={type}
          data-corridor-wall-style="dashed"
        >
          <g className="wall-main corridor-type-secret__wall-main">
            <path d={secretMainPath} />
          </g>
          <g className="wall-sketch corridor-type-secret__wall-sketch">
            <path d={secretSketchPath || secretMainPath} />
          </g>
        </g>
      );
    })
  ...
  return <g className="corridor-type-wall-accents">{rendered}</g>;
}

export function renderCorridorTypeAccents(generatedMap) {
  const corridors = Array.isArray(generatedMap?.corridors) ? generatedMap.corridors : [];
  const gridSize = generatedMap?.config?.gridSize || DEFAULT_CONFIG.gridSize;
  const rendered = corridors
    .map((corridor) => {
      ...
      const type = getCorridorType(corridor);
      const cells = getVisibleCorridorTopologyCells(corridor, generatedMap);
      const d = buildVisibleCorridorCenterlinePath(corridor, generatedMap, gridSize);
      if (!d || cells.length < 2) return null;
      if (type === "narrow") return null;
      if (type === "collapsed") {
        return (
          <g
...
export function getStairMarkerId(marker = {}) {
  const corridorId = String(marker.corridorId || "").trim();
  if (!corridorId) return "";
  const rawIndex = Number(marker.markerIndex ?? 0);
  const markerIndex = Number.isFinite(rawIndex) ? Math.max(0, Math.round(rawIndex)) : 0;
...
export function getMapStairMarkerEditorHandles(generatedMap, markerPositions = null) {
  ...
  return (generatedMap.corridors || []).flatMap((corridor) =>
    getCorridorStairMarkerVirtualDoors(corridor, generatedMap, markerPositions).map((door) => {
      ...
      const markerCount = Number(door.markerCount || 1);
      const transition = normalizeStairTransition(door.stairTransition, "none");
      return {
        id,
        corridorId: corridor.id,
        corridor,
        ...
        x: point.x,
        y: point.y,
        label: `${transition === "up" ? "Up" : "Down"} stair ${markerIndex + 1} of ${markerCount}`,
      };
    }),
  );
}
...
export function renderDerivedCorridorStairMarkers(
  ...
  if (rendered.length === 0) return null;
  return <g className="corridor-stair-markers">{rendered}</g>;
}
...
export function createCorridorSurface(
  ...
  const floorCells = getVisibleCorridorFloorCells(corridor, generatedMap);
  const pathCells = getVisibleCorridorTopologyCells(corridor, generatedMap);
  ...
  const narrowSurface =
    !organicPath &&
    getCorridorType(corridor) === "narrow" &&
    !corridor?.isRoomLink;
  const narrowFloorPath = narrowSurface
    ? buildNarrowCorridorFloorPath(corridor, generatedMap, pathCells, gridSize)
    : "";
  const visualFloorPath =
    organicPath ||
    narrowFloorPath ||
    buildFloorPath(floorCells, gridSize);
  const organicSurface = Boolean(organicPath);
  const geometryKind = organicSurface
    ? "organic-corridor-mask"
    : narrowFloorPath
      ? "narrow-corridor-mask"
      : "corridor-cell-mask";
  return {
    corridorId: corridor.id,
    ...
    <>
      ...
      <g className="corridor-texture">
        {corridors.map((corridor) => {
          if (getCorridorType(corridor) === "narrow") return null;
      ...
      {renderCorridorTypeAccents(generatedMap)}
    </>
  );
}
...
export function createPreciseWallPath(wall) {
  if (
...
export function usesPreciseWallDrawing(config = {}) {
  return config.wallDrawingStyle === "precise";
}

export function createRoughWallPath(wall, config, index, layer = "main") {
  if (usesPreciseWallDrawing(config)) return createPreciseWallPath(wall);
...
export function createRoughCircleWallPath(
  ...
  if (usesPreciseWallDrawing(generatedMap.config)) {
    return createCircleCompositeArcPath(region, generatedMap);
  }
  const circle = getCircleGeometryFromRegion(
    region,
    generatedMap.config.gridSize,
...
export function createRoughDoorPanelPath(rect, config, index) {
  ...
  if (usesPreciseWallDrawing(config)) {
...
export function createStairDirectionArrowSegments(
  door,
  generatedMap,
  stairTransition,
) {
  ...
  const travel = getDoorCorridorTravelDirection(door, generatedMap);
  const direction =
    transition === "up" ? { x: -travel.x, y: -travel.y } : travel;
  const tangent = { x: -direction.y, y: direction.x };
  const tail = {
    x: center.x - direction.x * gridSize * 0.16,
    y: center.y - direction.y * gridSize * 0.16,
  };
  const tip = {
    x: center.x + direction.x * gridSize * 0.46,
    y: center.y + direction.y * gridSize * 0.46,
  };
  const wingCenter = {
    x: tip.x - direction.x * gridSize * 0.18,
    y: tip.y - direction.y * gridSize * 0.18,
  };
  const wingSpan = gridSize * 0.18;
  return [
    { x1: tail.x, y1: tail.y, x2: tip.x, y2: tip.y },
    {
      x1: tip.x,
      y1: tip.y,
      x2: wingCenter.x + tangent.x * wingSpan,
      y2: wingCenter.y + tangent.y * wingSpan,
    },
    {
      x1: tip.x,
      y1: tip.y,
...
export function renderStairMark(
  ...
  return (
    <g
      ...
      data-stair-marker-id={markerId || undefined}
      data-corridor-id={door.corridorId || undefined}
...
export function getDrawableWallSegments(generatedMap) {
  ...
  const replaceableWalls = removeCorridorTypeReplacementWalls(
    [...gridWalls, ...corridorWallsNearCircles],
    generatedMap,
  );
  const baseWalls = trimWallSegmentsAgainstMineCaveOpenings(
    mergeCollinearWallSegments(replaceableWalls),
    generatedMap,
  );
...
export function renderUnifiedDungeonSurface(generatedMap, gridStyle = "solid", renderOptions = {}) {
  ...
  return (
    <>
      ...
      {renderCorridorTypeWallAccents(generatedMap)}
...
export function getRoomLevelBadgeLabel(region) {
  const level = getRegionLevel(region);
  if (!Number.isFinite(level) || level === 0) return "";
  return `L${formatLevelForBadge(level)}`;
}

function formatLevelForBadge(level) {
  const numeric = Number(level);
  ...
  return (
    <g className="labels">
      {generatedMap.regions.map((region) => {
        ...
        const levelBadgeLabel = getRoomLevelBadgeLabel(region);
        return (
                ...
                {levelBadgeLabel ? (
                  ...
                  >
                    ...
                    <rect
                      ...
                      y={region.labelPoint.y - 23}
                      width={24}
                      height={13}
                      rx={0}
                    />
                    <text
                      className="room-level-text"
                      x={region.labelPoint.x + 19}
                      y={region.labelPoint.y - 14}
                      textAnchor="middle"
                    >
                      {levelBadgeLabel}
                    </text>
    ...
    <g className="editor-overlays">
      ...
      {selectedStairMarker ? (
        <g
          ...
          data-stair-marker-id={selectedStairMarker.id}
          data-corridor-id={selectedStairMarker.corridorId}
          data-stair-marker-index={selectedStairMarker.markerIndex}
          aria-hidden="true"
        >
          <circle
            className="stair-marker-selection__halo"
            cx={selectedStairMarker.x}
            cy={selectedStairMarker.y}
            r={config.gridSize * 0.48}
          />
          <circle
            className="stair-marker-selection__edge"
            cx={selectedStairMarker.x}
            cy={selectedStairMarker.y}
            r={config.gridSize * 0.48}
          />
      ...
      <g className="stair-marker-hit-zones">
...
const HATCH_SHADOW_STROKES = Object.freeze({
  default: Object.freeze({
    cruor: "#050506",
    ink: "#dccaa6",
    cartographic: "#f4efe4",
  ...
  black: "rgba(0,0,0,.78)",
  blood: "rgba(51,3,12,.72)",
  bone: "rgba(240,215,220,.52)",
  none: "transparent",
});
...
function normalizeHatchShadowColor(value) {
  return Object.prototype.hasOwnProperty.call(HATCH_SHADOW_STROKES, value)
    ? value
    : "default";
}

function getHatchShadowStroke(value, visualStyle) {
  const normalized = normalizeHatchShadowColor(value);
  const stroke = HATCH_SHADOW_STROKES[normalized];
  if (typeof stroke === "string") return stroke;
  return stroke[normalizeVisualStyle(visualStyle)] || stroke.cruor;
}

function withWallDrawingConfig(map, wallDrawingStyle) {
  if (!map?.config) return map;
  const normalized = normalizeWallDrawingStyle(wallDrawingStyle);
  if (map.config.wallDrawingStyle === normalized) return map;
  return {
    ...map,
    config: {
      ...map.config,
      wallDrawingStyle: normalized,
    },
  };
}
...
export function MapSvg({
  ...
  const fadedSurfaceMap = React.useMemo(
    () => withWallDrawingConfig(fadedMap, normalizedWallDrawingStyle),
    [fadedMap, normalizedWallDrawingStyle],
  );
  const activeEditorSurfaceMap = React.useMemo(
    () => withWallDrawingConfig(activeEditorMap, normalizedWallDrawingStyle),
    [activeEditorMap, normalizedWallDrawingStyle],
  );
  const gridVisualVariables = {
  ...
  const staticMapContent = React.useMemo(
    ...
    [
      ...
      gridWeight,
      layerGridStyle,
      normalizedHatchShadowColor,
      normalizedWallDrawingStyle,
  ...
  return (
    <svg
      ...
      data-level-view={String(normalizedSvgLevelView)}
