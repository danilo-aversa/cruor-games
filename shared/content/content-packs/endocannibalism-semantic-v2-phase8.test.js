...
import { ENDOCANNIBALISM_INSPIRATION_MODULE } from "../inspiration-modules/endocannibalism.js";
...
import { ENDOCANNIBALISM_SEMANTIC_V2_MODULE, ENDOCANNIBALISM_SEMANTIC_V2_PACK } from "./endocannibalism-semantic-v2-pack.js";

function referencesSource(graft = {}) {
  return (graft.sourceAnchors?.length ? graft.sourceAnchors : [graft.source]).filter(Boolean).includes("endocannibalism");
}

describe("Phase 8 batch 8 — Endocannibalism approved Candidate 1", () => {
  it("owns only Archive and Dark Places with no Monster bridge or payload", () => {
    const source = readFileSync(resolve(process.cwd(), "shared/content/content-packs/endocannibalism-semantic-v2-pack.js"), "utf8");
    expect(source).not.toMatch(/MONSTER_GRAFTS/);
    expect(source).not.toMatch(/semanticType:\s*"monster-graft"/);
    expect(ENDOCANNIBALISM_SEMANTIC_V2_MODULE.capabilities).toEqual(["dark-places", "inspiration-archive"]);
    expect(ENDOCANNIBALISM_SEMANTIC_V2_MODULE.components.filter((entry) => entry.semanticType === "monster-graft")).toEqual([]);
  ...
  it("adds approved Candidate 1 to the semantic catalog without changing production", () => {
    const module = CRUOR_INSPIRATION_MODULES.find((entry) => entry.id === "endocannibalism");
    expect(module).toBe(ENDOCANNIBALISM_SEMANTIC_V2_MODULE);
    expect(module.schemaVersion).toBe("cruor-inspiration-module-v2");
    expect(module.status).toBe("in-review");
    expect(validateContentPackV0_2(ENDOCANNIBALISM_SEMANTIC_V2_PACK)).toEqual([]);
    expect(STATIC_CONTENT_REGISTRY.getInspirations({ workflow: "inspiration-archive" })).toHaveLength(14);
    expect(STATIC_CONTENT_REGISTRY.getComponents({ sourceAnchor: "endocannibalism" })).toHaveLength(11);
  });

  it("records approval provenance and the remaining image gate", () => {
    const records = [ENDOCANNIBALISM_SEMANTIC_V2_MODULE, ENDOCANNIBALISM_SEMANTIC_V2_MODULE.inspiration, ...ENDOCANNIBALISM_SEMANTIC_V2_MODULE.components];
    records.forEach((record) => expect(record.provenance.migration).toMatchObject({
      method: "editorially-migrated", editorialDecision: "approved", reviewVersion: "phase8-endocannibalism-editorial-approved-v1",
    }));
    expect(getInspirationV2MigrationRecord("endocannibalism")).toMatchObject({
      ...
      candidate: { reviewVersion: "phase8-endocannibalism-editorial-approved-v1", ownershipModel: "archive-dark-places-only" },
    });
  });

  it("keeps the source boundary explicit", () => {
    const notes = ENDOCANNIBALISM_SEMANTIC_V2_MODULE.sourceAnchor.editorialNotes.join(" ");
    const cautions = ENDOCANNIBALISM_SEMANTIC_V2_MODULE.inspiration.editorial.cautions.join(" ");
    expect(notes).toContain("South Fore");
  ...
  it("accounts for all 11 legacy location and region ids exactly once", () => {
    const legacyIds = ENDOCANNIBALISM_INSPIRATION_MODULE.components.map((component) => component.id);
    const migratedIds = ENDOCANNIBALISM_SEMANTIC_V2_MODULE.components.flatMap((component) => component.provenance.legacyIds);
  ...
  it("provides complete Dark Places semantic coverage", () => {
    const coverage = buildStudioSemanticCoverage(ENDOCANNIBALISM_SEMANTIC_V2_MODULE);
  ...
  it("compiles deterministic samples with zero diagnostics", () => {
    const report = runDarkPlacesSemanticSampleQa({ pack: ENDOCANNIBALISM_SEMANTIC_V2_PACK, module: ENDOCANNIBALISM_SEMANTIC_V2_MODULE });
