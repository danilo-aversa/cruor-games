...
import { JIKININKI_SEMANTIC_V2_MODULE, JIKININKI_SEMANTIC_V2_PACK } from "./jikininki-semantic-v2-pack.js";
...
describe("Phase 8 batch 14 — Jikininki approved Candidate 1", () => {
  it("owns only Archive and Dark Places while Monster parity remains external", () => {
    ...
    expect(JIKININKI_SEMANTIC_V2_MODULE.capabilities).toEqual(["dark-places", "inspiration-archive"]);
    expect(JIKININKI_SEMANTIC_V2_MODULE.components.filter((entry) => entry.semanticType === "monster-graft")).toEqual([]);
  ...
  it("records editorial approval and zero-diagnostic sample QA", () => {
    const records = [
      ...
      ...JIKININKI_SEMANTIC_V2_MODULE.components,
    ];
  ...
  it("keeps the source boundary explicit", () => {
    const notes = JIKININKI_SEMANTIC_V2_MODULE.sourceAnchor.editorialNotes.join(" ");
    const cautions = JIKININKI_SEMANTIC_V2_MODULE.inspiration.editorial.cautions.join(" ");
    expect(notes.toLowerCase()).toContain("1904");
  ...
  it("accounts for every Archive and Dark Places legacy id exactly once", () => {
    ...
    const migratedIds = JIKININKI_SEMANTIC_V2_MODULE.components.flatMap(
  ...
  it("provides complete Dark Places semantic coverage", () => {
    const coverage = buildStudioSemanticCoverage(JIKININKI_SEMANTIC_V2_MODULE);
  ...
  it("compiles deterministic samples with zero diagnostics", () => {
    const report = runDarkPlacesSemanticSampleQa({
      pack: JIKININKI_SEMANTIC_V2_PACK,
      module: JIKININKI_SEMANTIC_V2_MODULE,
    });
