...
## Site Topbar and Transient Navigation

### Purpose and canonical source

Use this family for the persistent site header, primary navigation, Crucible mega menu, Settings menu, and the shared workspace overlay displayed while either desktop menu is open.

- React shell owner: `app/AppShell.jsx`
- Topbar behavior: `app/navigation/SiteTopbar.jsx`
- Mega menu structure: `app/navigation/SiteMegaMenu.jsx`
- Shared styling: `app/app-shell.css` and `app/navigation/site-mega-menu.css`

### Interaction contract

- The brand control displays the Cruor mark only and retains an accessible home label.
- Hover, focus, click, keyboard activation, Escape dismissal, outside-click dismissal, and menu semantics remain owned by `SiteTopbar` and `SiteMegaMenu`.
- Crucible and Settings use the same transient-navigation overlay state owned by `AppShell`.
- The overlay starts below the topbar and fades with the menus. While it is present, `AppShell` temporarily lowers the Home section-progress diamonds beneath the overlay without creating a workspace-wide stacking context.
- The overlay must remain below the topbar and portaled mega menu.
- Menu entrance and exit transitions must respect the global reduced-motion settings.

### Class API

- `.site-topbar` and `.site-topbar__*` — persistent topbar structure and controls.
- `.site-mega-menu` and `.site-mega-menu__*` — Crucible menu and preview structure.
- `.app-shell__navigation-overlay` — shared darkened, blurred workspace overlay.
- `[data-transition-state="opening|open|closing"]` — transient menu presence state used for fades without abrupt unmounting.

### Stacking requirements

`.app-shell__navigation-overlay` remains below `.site-topbar` and the portaled `.site-mega-menu`. The root attribute `data-transient-navigation-open="true"` temporarily lowers `.cruor-home__scroll-progress` beneath the overlay for the full fade lifecycle. Do not isolate the entire workspace: feature modals and drawers must retain their existing document-level stacking behavior.
...
```html
<div
  class="cruor-dropdown-menu cruor-dropdown-menu--listbox"
  role="listbox"
  aria-label="Monster type"
>
  <button
    class="cruor-dropdown-option"
    type="button"
    role="option"
    aria-selected="true"
  >
...
Use this family for reusable assignment slots that display an icon and slot label, a value or count, a selected component title, and a short description. Terrifying Monsters silhouette slots are the canonical visual source.

- React primitive: `components/ui/composer-slot-card.jsx`
- Shared stylesheet: `shared/styles/composer-system.css`

### React API

`ComposerSlotCard` accepts `icon`, `label`, `value`, `contentTitle`, `description`, `filled`, `active`, and `className`. It forwards refs and remaining button attributes, so features can preserve ARIA semantics, test hooks, tooltips, pointer handlers, and data attributes.

### Class API

- `.cruor-composer-slot-card` — canonical button surface and layout.
- `.cruor-composer-slot-card__head` — icon, slot label, and trailing value.
- `.cruor-composer-slot-card__body` — selected component title and description.
- `.is-empty` / `.is-filled` — content state generated by the component.
- `.is-active` — selected slot state generated by the component.
- `.is-linked-hover` — reusable linked-diagram hover state when a feature needs it.

### Allowed feature-specific behavior

Feature code may supply layout modifiers such as `is-bottom`, workflow states such as `is-missing`, `is-suggested`, or `is-guided`, data attributes, refs, event handlers, tooltip hooks, and feature-required height or width overrides. These additions must not recreate the shared visual implementation.
...
<ComposerSlotCard
  ...
  filled={false}
  active={activeSlot === "body"}
  onClick={() => focusSlot("body")}
/>
```
